...
of_fb_winsize = np.mean(np.divide(s,30),dtype='int')
...
time.sleep(.1)
frame1 = cv2.cvtColor(vs.read()[1],cv2.COLOR_BGR2GRAY)
...
# If the possibility to track an arbitrary number of participants (ROIs) is added, then there should be a more intelligent way of declaring all the variables.
...
while(1):
    ...
    # Double the output variables' sizes when we reach their limits. That's because we don't know in advance how long the user will keep the video running.
    ...
    # That's a mystery shortcut in the loop with a secret purpose.
    if 0:
        continue
